<script lang="ts">
  ...
  function handleExerciseSelect(exercise: ExerciseType) {
    ...
    // Se for squat, redirecionar para framer
    if (exercise === 'squat') {
      goto('/framer');
    } else {
      // Para outros exercícios, manter comportamento original
      goto('/train/intro');
    }
  }
</script>
...
<style>
  ...
  :global(.exercise-disabled) {
    opacity: 0.4;
    cursor: not-allowed !important;
    pointer-events: none;
  }

  :global(.exercise-disabled img) {
    filter: grayscale(100%);
  }

  :global(.exercise-disabled .glass-overlay) {
    background: rgba(0, 0, 0, 0.6);
  }

  :global(.exercise-disabled .button-primary) {
    background: #6c757d !important;
  }

  :global(.exercise-disabled:hover .exercise-name) {
  ...
  <main class="w-full px-4 pb-4 pt-20 sm:pt-24">
    ...
    <div class="grid grid-cols-2 max-[420px]:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 w-full">
      {#each exercises as exercise}
        <div
          class="flex flex-col cursor-pointer group {exercise !== 'squat' ? 'exercise-disabled' : ''}"
          onclick={() => handleExerciseSelect(exercise)}
          onkeypress={(e) => e.key === 'Enter' && handleExerciseSelect(exercise)}
          role="button"
          tabindex="0"
          aria-label="Selecionar {$_(`train.${exercise}`)}"
        >
