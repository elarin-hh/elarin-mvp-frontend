...
export class ExerciseAnalyzer {
	...
	private async initializeMLClassifier(): Promise<void> {
		this.mlClassifier = new GenericExerciseClassifier(
			(this.config.mlConfig as any) || {}
		);

		const modelPath = this.config.modelPath || `./models/${(this.config as any).modelFile}`;
		const metadataFile = (this.config as any).metadataFile || null;
	...
	private async initializeHeuristicValidator(): Promise<void> {
		try {
			...
			if (!hasValidator(exerciseId)) {
				console.warn(`No validator registered for exercise: ${exerciseId}`);
				this.heuristicValidator = null;
				return;
			}
			...
			this.heuristicValidator = createValidator(
				exerciseId,
				(this.config.heuristicConfig as any) || {}
			);

			if (this.heuristicValidator) {
				console.log(`✅ Heuristic validator loaded for: ${exerciseId}`);
			}
		} catch (error) {
			console.error('Failed to initialize heuristic validator:', error);
			this.heuristicValidator = null;
		}
	}
	...
	private initializeFeedbackSystem(): void {
		this.feedbackSystem = new FeedbackSystem(
			(this.config.feedbackConfig as any) || {}
		);
	}
	...
	getConfig(): AnalyzerConfig {
		return {
			...
			feedbackMode: (this.feedbackSystem as any)?.config.feedbackMode,
			mlWeight: (this.feedbackSystem as any)?.config.mlWeight,
			heuristicWeight: (this.feedbackSystem as any)?.config.heuristicWeight,
			analysisInterval: this.analysisInterval,
			mlConfig: this.config.mlConfig as any,
			heuristicConfig: this.config.heuristicConfig as any
		};
	}
