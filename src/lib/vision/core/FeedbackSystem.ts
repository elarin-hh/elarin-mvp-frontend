...
export class FeedbackSystem {
	...
	integrate(mlResult: MLResult | null, heuristicResult: ValidationResult | null): FeedbackRecord {
		...
		// DEBUG: Log feedback decision
		logger.log('Feedback', 'Feedback Integration Result', {
			mode: feedback.mode,
			mlAvailable: feedback.ml.available,
			mlIsCorrect: feedback.ml.isCorrect,
			heuristicAvailable: feedback.heuristic.available,
			heuristicIsValid: feedback.heuristic.isValid,
			combinedVerdict: feedback.combined.verdict,
			combinedIsCorrect: feedback.combined.isCorrect,
			combinedConfidence: feedback.combined.confidence
		});
	...
	private processMLResult(mlResult: MLResult | null): ProcessedMLResult {
		...
		const confidence =
			mlResult.confidence !== undefined && !isNaN(mlResult.confidence)
				? mlResult.confidence
				: 0.7;
	 ...
	 */
	private processHeuristicResult(heuristicResult: ValidationResult | null): ProcessedHeuristicResult {
		if (!heuristicResult) {
			return {
				available: false,
				issues: []
	...
	private heuristicOnlyDecision(heuristicData: ProcessedHeuristicResult): CombinedDecision {
		...
		const criticalIssues = heuristicData.issues?.filter((i) => i.severity === 'critical').length || 0;
		const highIssues = heuristicData.issues?.filter((i) => i.severity === 'high').length || 0;
	 ...
	 */
	private hybridDecision(mlData: ProcessedMLResult, heuristicData: ProcessedHeuristicResult): CombinedDecision {
		...
		// Combina com pesos
		const combinedScore = mlScore * this.config.mlWeight + heuristicScore * this.config.heuristicWeight;
	...
	private generateMessages(feedback: FeedbackRecord): FeedbackMessage[] {
		...
		if (feedback.heuristic.available && feedback.heuristic.issues && feedback.heuristic.issues.length > 0) {
	 ...
	 */
	private prioritizeIssues(issues: ProcessedHeuristicResult['issues']): NonNullable<ProcessedHeuristicResult['issues']> {
		if (!issues) return [];
	 ...
	 */
	private getIssueFeedbackText(issue: NonNullable<ProcessedHeuristicResult['issues']>[number]): string {
